...
#define arena_alloc(arena, type) ((type*)arena_alloc_raw(arena, sizeof(type)))
// Defer a function with a passed in user_data type, pass void for no user_data
#define arena_defer(arena, func, type) ((type*)arena_defer_raw(arena, (arena_defer_t)func, sizeof(type)))
// Copy an allocated node in arena
#define arena_alloc_copy(arena, mem, type) ((type*)arena_alloc_copy_raw(arena, mem))